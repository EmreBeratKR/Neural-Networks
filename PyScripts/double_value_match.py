...
folder = "2025-11-24_07-08-37"
gen = 100

os.makedirs(f"double value match/{folder}", exist_ok=True)
os.makedirs(f"double value match/{folder}/dists", exist_ok=True)
os.makedirs(f"double value match/{folder}/solspace", exist_ok=True)

def fitness_values():
  with open(f"../Assets/Double Value Match/Saves/{folder}/gen_{gen}.json", "r") as f:
    data = json.load(f)
  ...
  #plt.show()
  #return
  plt.savefig(f"double value match/{folder}/mean_fitness_graph.png", dpi=300)
  plt.clf()
  plt.close()
...
def dot_distribution():
  for i in range(1, gen + 1):
    with open(f"../Assets/Double Value Match/Saves/{folder}/gen_{i}.json", "r") as f:
      data = json.load(f)

    population = data["entities"]
    values = [(p["actions"][0], p["actions"][1]) for p in population]
    ...
    for val, count in counts.items():
        for j in range(count):
            x.append((val[0] + val[1] * 256.0) / (256 * 256 - 1))
            y.append(j)
    ...
    #plt.show()
    #break
    plt.savefig(f"double value match/{folder}/dists/{i}.png", dpi=300)
    plt.clf()
    plt.close()
...
def solution_space():
  for i in range(1, gen + 1):
    ...
    for ind in range(0, len(x)):
      counts[(x[ind],y[ind])] = counts.get((x[ind],y[ind]), 0) + 1
    ...
    plt.xticks(xticks)
    plt.yticks(yticks)
    plt.title(f"2 Boyutlu Çözüm Uzayı ({i}.Nesil)")
    #plt.show()
    #break
    plt.savefig(f"double value match/{folder}/solspace/{i}.png", dpi=300)
    plt.clf()
    plt.close()
